		...
		<div className="py-20 lg:py-40 flex flex-col gap-10 px-4 ">
			<div className="mx-auto max-w-2xl md:text-center">
				...
				<p className="mt-4 text-lg tracking-tight text-muted-foreground text-center">
					...
					Turns out, deployit has actually helped a few people—who knew?
				</p>
			</div>
...
const grid = [
	{
		title: "GitHub Stars",
		description: `With over ${(statsValues.githubStars / 1000).toFixed(1)} stars on GitHub, deployit is trusted by developers worldwide. Explore our repositories and join our community!`,
		icon: (
	...
	},
	{
		...
		description: `Downloaded over ${(statsValues.dockerDownloads / 1000).toFixed(0)}k times, deployit has become a go-to solution for seamless deployments. Discover our presence on DockerHub.`,
		icon: (
			<svg
				stroke="currentColor"
				fill="currentColor"
	...
	},
	{
		...
		description: `Thanks to a growing base of over ${statsValues.contributors} contributors, deployit continues to thrive with valuable contributions from developers around the world.`,
	...
	{
		title: "Sponsors",
		description: `More than ${statsValues.sponsors} companies/individuals have sponsored deployit, ensuring a steady flow of support and resources. Join our community!`,
...
export function GridPattern({ width, height, x, y, squares, ...props }: any) {
	...
	// Convert squares to a Set to remove duplicates
	const uniqueSquares = Array.from(
		new Set(squares.map(([x, y]: [number, number]) => `${x}-${y}`))
	).map((coord) => 
		//@ts-ignore
		...
		<svg aria-hidden="true" {...props}>
			<defs>
				<pattern
					...
					y={y}
				>
					<path d={`M.5 ${safeHeight}V.5H${safeWidth}`} fill="none" />
				</pattern>
			</defs>
			<rect width="100%" height="100%" strokeWidth={0} fill={`url(#${patternId})`} />
			{uniqueSquares.map(([x, y]: [number, number]) => (
				<rect
					strokeWidth="0"
					key={`${x}-${y}`}
					width={safeWidth + 1}
